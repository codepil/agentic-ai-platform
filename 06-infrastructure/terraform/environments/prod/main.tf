...
#
# Wires together all platform modules for the production environment.
# Prod is configured for resilience and performance:
#   • Two AZs for high availability (ALB, ECS, VPC subnets)
#   • M30 MongoDB cluster with PIT recovery and cloud backup
#   • Two ECS tasks per service (no single point of failure)
#   • Mock mode disabled (real LLM API calls)
...
module "vpc" {
  ...
  availability_zones    = var.availability_zones    # two AZs in prod
...
module "ecs" {
  ...
  mock_mode = var.mock_mode  # "false" — real LLM API calls in prod
...
module "mongodb" {
  ...
  instance_size = var.mongo_instance_size  # M30
...
# -----------------------------------------------------------------------------
# Outputs — surfaced at the environment level
# -----------------------------------------------------------------------------
